...
import {
	...
	getCasePlayById,
	updateCasePlay
} from '$lib/server/db/repositories/case-plays';
import { listReferenceData } from '$lib/server/db/repositories/reference-data';
import { readDifficulty, readOptionalId, readOptionalText, readRequiredText } from '$lib/server/forms';
...
export const load = (async ({ params }) => {
	const [casePlay, references] = await Promise.all([
		getCasePlayById(params.id),
		listReferenceData()
	]);
