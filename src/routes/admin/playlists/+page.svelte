...
<section class="space-y-8">
	...
	<section class="border border-stone-300 bg-white p-5 shadow-sm">
		...
		<form method="POST" use:namedActionForm={{ name: 'create', resetOnSuccess: true }} class="mt-4 flex flex-col gap-3 sm:flex-row">
			<input name="title" required placeholder="Playlist title" class="flex-1 border border-stone-300 px-3 py-2" />
			<button class="cursor-pointer border border-stone-900 bg-stone-900 px-4 py-2 font-semibold text-white hover:bg-stone-700"> Create </button>
		</form>
	</section>
	...
	<div class="space-y-6">
		{#if data.playlists.length > 0}
			{#each data.playlists as playlist}
				...
				<section class="border border-stone-300 bg-white p-5 shadow-sm">
					<form method="POST" use:namedActionForm={'save'} class="space-y-4">
						<input type="hidden" name="playlistId" value={playlist.id} />
						<div class="grid gap-4 md:grid-cols-2">
							<div class="space-y-1">
								<label class="text-sm font-semibold" for="title-{playlist.id}">Title</label>
								<input id="title-{playlist.id}" name="title" value={playlist.title} class="w-full border border-stone-300 px-3 py-2" />
							</div>
							<div class="space-y-1">
							...
							<div class="max-h-96 space-y-2 overflow-y-auto border border-stone-200 p-3">
								{#each data.casePlayOptions as casePlay}
									<div class="grid grid-cols-[auto,1fr,90px] items-center gap-3 border border-stone-200 px-3 py-2">
										<input type="checkbox" name="selectedCasePlayIds" value={casePlay.id} checked={entries.has(casePlay.id)} />
										<div>
					...
					<form method="POST" use:namedActionForm={'delete'} class="mt-4">
						<input type="hidden" name="playlistId" value={playlist.id} />
