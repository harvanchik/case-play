...
export const load: PageServerLoad = async ({ url }) => {
	...
	const imageUrl = playBuilderId
		? `/diagram/flag-football/${playBuilderId}/social.png?play=${activePlayNumber}&studio=${Date.now()}`
		: `/diagram/flag-football/social.png?studio=${Date.now()}`;

	return { requestedId, playBuilderId, activePlayNumber, playCount: document.plays.length, imageUrl, message };
};
