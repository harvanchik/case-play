...
import { getCasePlayById, listCasePlays } from '$lib/server/db/repositories/case-plays';
import { rankSimilarCasePlays } from '$lib/server/case-play-similarity';

export const load = (async ({ params, url }) => {
	const id = params.casePlayId;
	const [casePlay, casePlays] = await Promise.all([getCasePlayById(id), listCasePlays()]);
